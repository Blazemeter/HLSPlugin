...
public enum ResolutionOption {
    CUSTOM {
        ...
        @Override
        public boolean matches(String resolution, String lastMatch, String customResolution) {
            return lastMatch == null || resolution != null && resolutionCompare(resolution, lastMatch) <= 0;
        }
    }, MAX {
        @Override
        public boolean matches(String resolution, String lastMatch, String customResolution) {
    ...
    public static ResolutionOption fromString(String str) {
        return str != null && !str.isEmpty() ? ResolutionOption.valueOf(str.substring(0, str.length() - STRING_SUFFIX.length()).toUpperCase()) : MIN;
    }
